...
rem  time (the copy bundled in this repo is missing its .jar files, so it
rem  cannot run), then compiles the plugin into a .jar.
rem ====================================================================
...
if not exist "%MVNCMD%" (
  echo [*] Downloading Apache Maven %MVNVER% (one-time) ...
  ...
  if errorlevel 1 ( echo [!] Download failed. Check your internet and re-run. & pause & exit /b 1 )
  echo [*] Unzipping ...
  powershell -NoProfile -Command "Expand-Archive -Force '%MVNZIP%' '%ROOT%tools'"
  del "%MVNZIP%" >nul 2>&1
)
if not exist "%MVNCMD%" ( echo [!] Maven still missing after unzip. & pause & exit /b 1 )

rem --- Build the plugin ---
echo [*] Building KawaiiCompanion (downloads the Paper library on first run) ...
